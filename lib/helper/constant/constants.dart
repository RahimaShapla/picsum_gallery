import 'package:flutter/material.dart';


//current app releasing mode
const MODE_DEVELOPMENT = true;

const Color kWhiteColor = Color(0xFFffffff);
const Color kWhiteColor1 = Color(0xFFF4F6FF);
const Color kAshColor = Color(0xFFEAF0F2);
const Color kLightGreyColor = Color(0xFFbfbfbf);
const Color kDarkGreyColor = Color(0xFF829197);
const Color kBlack = Color(0xFF000000);
const Color kTransparent = Color(0xFF00000000);
const Color kPrimaryColor = Color(0xFF267055);
const Color kPrimaryDarkColor = Color(0xFF1B4E3B);
const Color kAccentColor = Color(0xFF32926F);
const Color kMediumMint = Color(0xFF3EB489);
const Color kMildMint = Color(0xFF5AC69F);
const Color kLightMint = Color(0xFF9EDEC6);


class DefaultValue {
  static const bool kDefaultBoolean = false;
  static const int kDefaultInt = 0;
  static const double kDefaultDouble = 0.0;
  static const String kDefaultString = '';
}

class TextSize {
  static const double Text_Size_10 = 10;
  static const double Text_Size_12 = 12;
  static const double Text_Size_14 = 14;
  static const double Text_Size_16 = 16;
  static const double Text_Size_18 = 18;
  static const double Text_Size_20 = 20;
  static const double Text_Size_24 = 24;
  static const double Text_Size_30 = 30;
  static const double Text_Size_36 = 36;
}

class DimenHeight {
  static const double height_10 = 10;
  static const double height_15 = 15;
  static const double height_20 = 20;
  static const double height_25 = 25;
  static const double height_30 = 30;
  static const double height_35 = 35;
  static const double height_40 = 40;
  static const double height_45 = 45;
  static const double height_47 = 47;
  static const double height_50 = 50;
  static const double height_55 = 55;
  static const double height_70 = 70;
  static const double height_80 = 80;
  static const double height_96 = 96;
  static const double height_100 = 100;
  static const double height_130 = 130;
  static const double height_250 = 250;
  static const double height_200 = 200;
}


class PreferenceValue {}


